...
def word_freq(word_list):
    ...
    word_list should be a 2d list, e.g.WordList_in_SentenceList
    '''   
    word_cnt = {}
...
def unit2_dictionary(str,word_list):
    '''
    Check each word in word_list, whether the word(substring) is in the string and count how many times it appears. Return a dictionary
    str: the string 
    word_list: list
    '''
    counts = dict()
...
def cooccurrence_network(dictionary_of_bigrams,original_df):
    '''
    ...
    '''
    tweet_words=token_japanese_sentence(original_df['sentence'])
    one_word_list=flatten_list(tweet_words)
    d = dictionary_of_bigrams.set_index('bigram').T.to_dict('records')
    # Create network plot 
    G = nx.Graph()
...
def create_word_network(word_cnt_df,original_df,one_word_cnt,cnt=3):
    '''
